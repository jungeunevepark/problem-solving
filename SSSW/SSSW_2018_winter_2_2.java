/*
 * 다음 두 함수를 이용하여 아기상어가 물고기를 더 이상 먹을 수 없을 때까지 움직인 칸의 수를 구하는 문제
 * 다음 먹을 물고기가 있는 칸 구하는 함수
	1. 아기상어보다 크기가 작은 것 중에서
	2. 아기상어를 제외하고
	3. 같은 것이 있다면 가장 가까운 것! -> 가장 위쪽 -> 가장 왼쪽
	
	아기상어가 물고기를 향해 가는 함수
	1. 아기상어보다 크기가 큰 물고기가 있는 칸은 제외
	2. 상하좌우로만 움직일 수 있음
	3. 지나는 칸의 수가 최소여야 함.
	4. 물고기를 먹으면 배열을 0으로 초기화
	5. 아기상어의 크기는 자신의 크기와 같은 수의 물고기를 먹을 때 크기가 1 증가한다.
 * */
...
public class SSSW_2018_winter_2_2 {
	public static void main(String[] args) throws Exception{
		...
		while(true) {
			for(int i=0; i<N; i++)
				for(int j=0; j<N; j++)
					if(map[i][j] == 9) {sx = i; sy = j;}						// 매번 아기상어의 현재 위치를 갱신한다.
			move = move(sx, sy, N, size, map);									// 함수를 통해 아기상어가 몇 칸 움직였는지 return 변수 값을 받는다.
			if (move == 0) break;												// 변수값이 0인 경우 움직이지 않았으므로 while을 종료하고 결과값을 출력한다.
			result += move; eat++;
			if (eat == size) {size++; eat = 0;}									// 먹은 물고기의 수와 아기상어의 크기가 같으면 아기상어 크기를 키우고 물고기의 수를 0으로 바꾼다.
		}
	...
	static int find(int x, int y, int nx, int ny, int N, int size, int[][] map) {
		...
		while(!q.isEmpty()) {
			Node p = q.poll();
			int i = p.x; int j = p.y; int count = p.count;
			if(i == nx && j == ny) return count;									// 현재 위치에 도착한 경우 count값을 리턴하고 함수 종료
			...
			for(int j=0; j<N; j++) {
				if(map[i][j] != 0 && map[i][j] != 9 && map[i][j] < size) {								// 배열의 모든 위치를 조사하여 아기상어보다 크기가 작은 물고기를 발견하면
					...
					if (n.count > next && next < N*N) {												// n의 칸수 보다 작은 경우 n을 갱신해준다.
						n = new Node(i, j, next);}										// Tip : 이중 loop에서 이미 가장 위 -> 가장 왼쪽 순으로 조사하고 있으므로 같은 경우는 먼저 조사된 경우를 반영
				}																		// 즉 두 값이 같은 경우는 고려하지 않아도 된다.
			}
		}

		if(n.x == x && n.y == y) return 0;												// 아기상어가 이동하지 않는다.
		map[n.x][n.y] = 9; map[x][y] = 0;
